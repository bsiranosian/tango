...
jdVar <- sort(apply(jd, 2, var),decreasing=T)
...
#by var
plot(jd[20:29,names(jdVar[1])], type='o', col=cp[1], ylim=c(0,0.02))
for (i in 2:num){
  lines(jd[20:29,names(jdVar[i])], type='o', col=cp[i])
}
legend(1, 0.02, names(jdVar[1:num]), cex=0.5, col=cp, lty=1)
