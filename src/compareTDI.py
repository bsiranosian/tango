...
parser.add_argument("dataFile", action="store", metavar="saveFile", help="specify a filename to save resulting data to. Format will be CSV with the first row representing the start of each window. Data for each phage will be plottd on a new line. Z-scores for each window are the values in the file.")
...
from tetranucleotideAnalysis import doTDI
import numpy as np
import csv

#compareTDI(): computes tetranucleotide difference index z-scores for all genomes defined in nameFile. 
...
def compareTDI(nameFile, dataFile, xScale, subset, windowSize, stepSize, k, plotFile, title, maxNum):
	...
	with open(dataFile, 'wb') as sf:
		...
		" NEED TO FIND LONGEST WINDOW TO WRITE HEADER "
...
if __name__ == '__main__':
	compareTDI(nameFile, dataFile, xScale, subset,windowSize,stepSize,k,plotFile,title,maxNum)